...
class MapSewer extends MapWaterLevel{
    constructor(option){
		if(option.siteKey == null) option.siteKey = "no";
    ...
    LoadLayer(param){
		if(!this.map) return;
		if(!g_APP.waterLevelOption.showSewer) return;
    ...
    UpdateInfoWindow(d){
		...
		if(d.num){	//cluster
		    ...
		    str += "<p>時間 "+d.time+" </p>";
		    str += "<div class='info-bt-container'><div class='info-bt' onclick='g_APP.mapControl.OpenLineChart(\"sewer\");'>今日變化</div></div>";
